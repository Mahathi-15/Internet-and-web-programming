<?php
	...
	//Display songs in Ascending order
...
<body>
	...
	<header>
		<div class="container">
			<nav class="navbar navbar-expand-lg navbar-light">
                ...
                <li class="nav-item">
                    ...
                    <p style="color:blue;">| Kanada Songs |</p>
                </li>
      ...
      aria-haspopup="true" aria-expanded="false">Track</button>
								<div class="dropdown-menu dropdown-primary">
									<a class="dropdown-item" href="kannada_songs.php"><b>Kanada Songs</b></a>
		...
		<p>Search song:</p>
		<form method="POST" id="searchbar" style="margin-top:20px; margin-left:1000px; padding: 20px" action="">
			<input type="text" name ="search" placeholder="Search">
			<input type="submit" name ="submit">
		</form>

		<p>Seacrh using lyrics</p>
		<form method="POST" id="Lyricssearchbar" style="margin-top:20px; margin-left:1000px; padding: 20px" action="">
			<input type="text" name ="lyricsSearch" placeholder="Search">
			<input type="submit" name ="submit">
		</form>
	</header>
	<!-- //header -->
	...
	if(isset($_POST['search'])) {
		$search=$_POST['search'];
		if(strlen($search)>0){
			$mysearch = implode('', array($search, "%"));
			//$mysearch = $search."%";
			echo "The value of search is $search";
			$sql ="SELECT * FROM kannada_albums WHERE song_name like '$mysearch' ORDER BY song_id";
		}
		else{
	...
	else if(isset($_POST['lyricsSearch'])) {
		$search=$_POST['lyricsSearch'];
		if(strlen($search)>0){
			//$mysearch = implode('', array($search, "%"));
			$mysearch = "%".$search."%";
			echo "The value of search is $search";
			$sql ="SELECT * FROM kannada_albums WHERE Lyrics like '$mysearch' ORDER BY song_id";
		}
		else{
			echo "Search is empty";
			$sql = "SELECT * FROM kannada_albums ORDER BY song_id ASC";
