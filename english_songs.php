		...
		<?php
			...
			if(isset($_POST['search'])) {
				...
				if(strlen($search)>0){
					...
					$sql ="SELECT * FROM english_albums WHERE song_name like '$mysearch' ORDER BY song_id";
				}
				else{
				...
				if(strlen($search)>0){
					...
					$mysearch = "%".$search."%";
					echo "The value of search is $search";
					$sql ="SELECT * FROM english_albums WHERE Lyrics like '$mysearch' ORDER BY song_id";
				}
				else{
					echo "Search is empty";
					$sql = "SELECT * FROM english_albums ORDER BY song_id ASC";
