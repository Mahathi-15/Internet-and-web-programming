<?php
	...
	while($rows = mysqli_fetch_array($result)){
		...
		//Checking if favourite button is being clicked
		...
		<?php
			...
			if(isset($_POST['search'])) {
				...
				if(strlen($search)>0){
					...
					$sql ="SELECT * FROM user WHERE song_name like '$mysearch' ORDER BY song_id";
				}
				else{
					echo "Search is empty";
					$sql = "SELECT * FROM user ORDER BY song_id ASC";
					}
			}
				...
				if(strlen($search)>0){
					...
					$mysearch = "%".$search."%";
					echo "The value of search is $search";
					$sql ="SELECT * FROM user WHERE Lyrics like '$mysearch' ORDER BY song_id";
				}
				else{
					echo "Search is empty";
					$sql = "SELECT * FROM user ORDER BY song_id ASC";
